...
import { activateStartOfTurnItems } from "./phases/turn-start/start-of-turn-item-activation";
...
export function runTurnStart(state: BattleFieldState): BattleFieldState {
    let nextState = state.clone();
    nextState.turnNumber++;
    let startOfTurnState = applyTransforms(nextState, [
        applyPlayerSwitchIns,
        applyCpuSwitchIns,
        applyFieldHazards,
        applyStartOfTurnAbilities,
        activateStartOfTurnItems,
    ]);

    if (startOfTurnState.length > 1) {
        console.error('Multiple possible states after turn start', startOfTurnState);
        throw new Error('Multiple possible states after turn start');
    }

    return startOfTurnState[0].state;
}
